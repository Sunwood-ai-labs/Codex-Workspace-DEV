...
- `-a auto-edit` オプションでファイル編集を自動承認（コマンドは手動承認）
- `-a full-auto -w <path>` オプションでファイル編集とコマンドを自動承認し、サンドボックスをバイパス（書き込み可能なルートの指定が必須）
- サンドボックスを無効化するには、環境変数 `FORCE_DISABLE_SANDBOX=true` を指定
- すべての処理は日本語プロンプトで実行
